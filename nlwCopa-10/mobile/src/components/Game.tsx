...
export function Game({ data, setFirstTeamPoints, setSecondTeamPoints, onGuessConfirm }: Props) {
  ...
  const dataTeste = dayjs(Date()).locale(ptBR).format("YYYY-MM-DDTHH:mm:ss.000[Z]");
    ...
    >
...

      {
        ...
        <Button 
          size="xs" 
          w="full" 
          bgColor={dataTeste > data.date ? "gray.500" : "green.500"} 
          mt={4} 
          ...
          <HStack alignItems="center">
            <Text 
              color={dataTeste > data.date ? "gray.400" : "white"} 
              fontSize="xs" 
              fontFamily="heading" 
              mr={3}
            >
              {dataTeste > data.date ? 'TEMPO ESGOTADO' : 'CONFIRMAR PALPITE'} 
            </Text>
